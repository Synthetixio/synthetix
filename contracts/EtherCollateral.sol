...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address _owner, address _resolver) public Owned() Pausable(_owner) MixinResolver(_owner, _resolver) {
        ...
        )
    {
        ...
        _ethBalance = 0;// address(this).balance; // TODO add support for 0x47 SELFBALANCE, or fix this contract
    ...
    function _closeLoan(address account, uint256 loanID) private {
        ...
        require(synthsUSD().balanceOf(address(depot())) >= totalFees, "The sUSD Depot does not have enough sUSD to buy for fees");

...
task('interact', 'Interact with a deployed Synthetix instance from the command line')
	...
	.addOptionalParam('ethNetwork', 'The network to run off', 'mainnet')
	...
	.setAction(async taskArguments => {
		// Hardhat already uses the "network" param, so we use "ethNetwork" instead
		const network = taskArguments.ethNetwork;

		if (!synthetix.networks.includes(network)) {
			throw new Error(
				`Invalid network "${network}". Sould be one of: ${synthetix.networks.join(', ')}`
			);
		}

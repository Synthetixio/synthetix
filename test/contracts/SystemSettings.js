...
contract('SystemSettings', async accounts => {
	...
	const itCanSetCrossDomainMessageGasLimit = () => {
	...
	const itCanSetWaitingPeriodSecs = () => {
	...
	const itCanSetPriceDeviationThresholdFactor = () => {
	...
	const itCanSetFeePeriodDuration = () => {
	...
	const itCanSetLiquidationParams = () => {
	...
	const itCanSetExchangeFeeRates = () => {
	...
	const itCanSetMinimumStakeTime = () => {
	...
	const itCanSetAggregatorWarningFlags = () => {
	...
	const itCanSetEtherWrapperParams = () => {
	...
	const itCanSetAtomicExchangeParams = () => {
		...
		describe('setAtomicEquivalentForDexPricing', () => {
			...
			describe('when successfully invoked', () => {
				...
				it('allows to be reset', async () => {
					await systemSettings.setAtomicEquivalentForDexPricing(sETH, ZERO_ADDRESS, {
						from: owner,
					});
	...
	describe('Using SystemSettings', () => {
		...
		itCanSetCrossDomainMessageGasLimit();

		itCanSetTradingRewards();

		itCanSetWaitingPeriodSecs();

		itCanSetPriceDeviationThresholdFactor();

		itCanSetIssuanceRatio();

		itCanSetFeePeriodDuration();

		itCanSetTargetThreshold();

		itCanSetLiquidationParams();

		itCanSetStalePeriods();

		itCanSetExchangeFeeRates();

		itCanSetMinimumStakeTime();

		itCanSetAggregatorWarningFlags();

		itCanSetEtherWrapperParams();
	});

	describe('Using SystemSettingsL1', () => {
		before(async () => {
			({ SystemSettings: systemSettings } = await setupAllContracts({
				accounts,
				contracts: ['SystemSettingsL1'],
			}));
		});

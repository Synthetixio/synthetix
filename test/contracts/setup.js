...
const setupContract = async ({
	...
	const defaultArgs = {
		GenericMock: [],
		SynthetixL1ToL2Bridge: [owner, tryGetAddressOf('AddressResolver')],
		SynthetixL2ToL1Bridge: [owner, tryGetAddressOf('AddressResolver')],
...
const setupAllContracts = async ({
	...
	const baseContracts = [
		...
		{
			contract: 'SynthetixL1ToL2Bridge',
			mocks: ['ext:Messenger', 'alt:SynthetixOptimisticBridge'],
			deps: ['AddressResolver', 'Issuer', 'RewardEscrow'],
		},
		{
			contract: 'SynthetixL2ToL1Bridge',
			mocks: ['ext:Messenger', 'alt:SynthetixOptimisticBridge'],

const { artifacts, contract, legacy } = require('@nomiclabs/buidler');
...
contract('ExternStateToken', async accounts => {
	...
	const ProxyERC20 = artifacts.require(`ProxyERC20`, { useLegacy: true });
	const TokenState = artifacts.require(`TokenState`, { useLegacy: true });

	describe(
		legacy
			? 'when integrating with legacy contracts (ProxyERC20, TokenState)'

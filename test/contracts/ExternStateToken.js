const { artifacts, contract, network } = require('@nomiclabs/buidler');
...
contract('ExternStateToken', async accounts => {
	...
	[true, false]
		// support legacy except during coverage runs
		.filter(isLegacy => !isLegacy || network.name !== 'coverage')
		.forEach(isLegacy => {
			const suffix = isLegacy ? '_Legacy' : '';

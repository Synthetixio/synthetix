...
contract('Liquidations', accounts => {
	...
	describe('with issuanceRatio of 0.125', () => {
		...
		describe('calculateAmountToFixCollateral', () => {
			...
			describe('given target ratio of 800%, collateral of $600, debt of $300', () => {
				...
				describe('given liquidation penalty is 10%', () => {
					...
					it('calculates mimicUSD to fix ratio from 200%, with $600 MIME collateral and $300 debt', async () => {
					...
					it('calculates mimicUSD to fix ratio from 300%, with $600 MIME collateral and $200 debt', async () => {
		...
		describe('when anyone calls liquidateDelinquentAccount on alice', () => {
			...
			describe('when Alice is undercollateralized', () => {
				...
				describe('when Bob flags Alice for liquidation', () => {
					...
					describe('when deadline has passed and Alice issuance ratio is fixed as MIME price increases', () => {
						beforeEach(async () => {
					...
					describe('when the price of MIME increases', () => {
					...
					describe('given the liquidation deadline has passed ', () => {
						...
						describe('when Alice c-ratio is above the liquidation Ratio and Bob liquidates alice', () => {
							...
							it('then Bob still has 0 MIME', async () => {
								assert.bnEqual(await synthetix.balanceOf(bob), 0);
							});
							it('then Alice still has 800 MIME', async () => {
						...
						describe('when Alice does not fix her c-ratio ', () => {
							...
							it('when carol calls liquidateDelinquentAccount but has 0 mimicUSD then revert', async () => {
							...
							describe('when Bobs liquidates alice for 100 mimicUSD but only has 99 mimicUSD then revert', async () => {
							...
							describe('when Bob liquidates alice for 100 mimicUSD to get 110 MIME', () => {
								...
								it('then Bob mimicUSD balance is reduced by 100 sUSD', async () => {
								...
								it('then Alice has less MIME + penalty', async () => {
								...
								describe('given carol has obtained mimicUSD to liquidate alice', () => {
									...
									describe('when carol liquidates Alice with 10 x 5 sUSD', () => {
										...
										it('then Carols mimicUSD balance is reduced by 50 sUSD', async () => {
											assert.bnEqual(await sUSDContract.balanceOf(carol), 0);
										});
										...
										it('then Carol has extra 50 MIME + the 5 MIME penalty (55)', async () => {
											const snxBalance = await synthetix.balanceOf(carol);
											assert.bnEqual(snxBalance, carolSNXBefore.add(SNX55));
										});
										it('then Alice MIME balance is 635', async () => {
											const aliceSNXAfter = await synthetix.collateral(alice);
									...
									describe('when carol liquidates Alice with 50 sUSD', () => {
										...
										describe('when Bob liqudates Alice with 1000 sUSD', () => {
											...
											it('then Bobs partially liquidates the 1000 mimicUSD to repair Alice to target issuance ratio', async () => {
							...
							describe('given Alice has $600 Debt, $800 worth of MIME Collateral and c-ratio at 133.33%', () => {
								describe('when bob calls liquidate on Alice in multiple calls until fixing the ratio', () => {
									...
									it('then Bob can liquidate Alice multiple times until fixing the c-ratio', async () => {
										...
										// Bob's mimicUSD balance should be less amountToFixRatio
										assert.bnEqual(
		...
		describe('Given Alice has MIME and never issued any debt', () => {
			beforeEach(async () => {
		...
		describe('When David collateral value is less than debt issued + penalty) ', () => {
			...
			describe('when Bob flags and tries to liquidate David', () => {
				beforeEach(async () => {
					...
					// Drop MIME value to $0.1 after update rates resets to default
					await updateSNXPrice('0.1');
				...
				describe('when the MIME rate is stale', () => {
					beforeEach(async () => {

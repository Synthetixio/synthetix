...
contract('VirtualSynth (unit tests)', async accounts => {
	...
	describe('with common setup', () => {
		...
		describe('using base deployment', () => {
			behaviors.whenInstantiatedAsBase(() => {
				...
				it('and the state is empty', async () => {
					assert.equal(await this.instance.synth(), ZERO_ADDRESS);
					assert.equal(await this.instance.resolver(), ZERO_ADDRESS);
					assert.equal(await this.instance.totalSupply(), '0');
					assert.equal(await this.instance.balanceOf(owner), '0');
					assert.equal(await this.instance.balanceOfUnderlying(owner), '0');
					assert.equal(await this.instance.currencyKey(), ZERO_BYTES32);
					assert.equal(trimUtf8EscapeChars(await this.instance.name()), 'Virtual Synth ');
					assert.equal(trimUtf8EscapeChars(await this.instance.symbol()), 'v');
				});

				it('and state-dependent functions fail', async () => {
					await assert.revert(this.instance.secsLeftInWaitingPeriod());
					await assert.revert(this.instance.readyToSettle());
				});
			});
		});

		describe('using proxied deployment', () => {
			describe('initialize', () => {
				const amount = '1001';
				behaviors.whenInstantiatedAsProxy({ amount, user: owner, synth: 'sBTC' }, () => {
					it('is initialized', async () => {
						assert.isTrue(await this.instance.initialized());
					});
			...
			describe('balanceOfUnderlying()', () => {
				const amount = '1200';
				behaviors.whenInstantiatedAsProxy({ amount, user: owner }, () => {
						...
						behaviors.whenMockedSettlementOwing({ rebate: 300 }, () => {
							...
							behaviors.whenUserTransfersAwayTokens(
								...
								() => {
									it('then balance underlying must match the balance after the reclaim, in proportion to their share', async () => {
										assert.equal(
											await this.instance.balanceOfUnderlying(owner),
											(+amount + 300) / 2
										);
									});
			...
			describe('rate()', () => {
				const amount = '1200';
				behaviors.whenInstantiatedAsProxy({ amount: '0', user: owner }, () => {
			...
			describe('secsLeftInWaitingPeriod()', () => {
				behaviors.whenInstantiatedAsProxy({ amount: '1000', user: owner, synth: 'sBTC' }, () => {
			...
			describe('readyToSettle()', () => {
				behaviors.whenInstantiatedAsProxy({ amount: '999', user: owner, synth: 'sBTC' }, () => {
					behaviors.whenMockedWithMaxSecsLeft({ maxSecsLeft: 100 }, () => {

...
contract('SystemStatus', async accounts => {
	...
	it('not even the owner can suspend', async () => {
		...
		await assert.revert(
			systemStatus.suspendSynthExchange(toBytes32('mimicETH'), '1', { from: owner }),
			'Restricted to access control list'
		);
		await assert.revert(
			systemStatus.suspendSynth(toBytes32('mimicETH'), '1', { from: owner }),
			'Restricted to access control list'
		);
	});
	...
	describe('when the owner is given access to suspend and resume everything', () => {
		...
		describe('suspendSystem()', () => {
			...
			it('and all the require checks succeed', async () => {
				...
				await systemStatus.requireSynthActive(toBytes32('mimicETH'));
				await systemStatus.requireSynthsActive(toBytes32('sBTC'), toBytes32('mimicETH'));
			});
			...
			describe('when the owner suspends', () => {
				...
				it('and the require checks all revert as expected', async () => {
					...
					await assert.revert(systemStatus.requireSynthActive(toBytes32('mimicETH')), reason);
					await assert.revert(
						systemStatus.requireSynthsActive(toBytes32('sBTC'), toBytes32('mimicETH')),
						reason
					);
				});
			});
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend with upgrading', () => {
					...
					it('nor can it do any other restricted action', async () => {
						...
						await assert.revert(
							systemStatus.suspendSynth(toBytes32('mimicETH'), '0', { from: account1 })
						);
						await assert.revert(systemStatus.resumeSynth(toBytes32('mimicETH'), { from: account1 }));
					});
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('nor can it do any other restricted action', async () => {
							...
							await assert.revert(systemStatus.resumeIssuance({ from: account1 }));
							await assert.revert(
								systemStatus.suspendSynth(toBytes32('mimicETH'), '66', { from: account1 })
							);
							await assert.revert(systemStatus.resumeSynth(toBytes32('mimicETH'), { from: account1 }));
						});
					});
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('nor can it do any other restricted action', async () => {
						...
						await assert.revert(
							systemStatus.suspendSynth(toBytes32('mimicETH'), '55', { from: account2 })
						);
						await assert.revert(systemStatus.resumeSynth(toBytes32('mimicETH'), { from: account2 }));
					});
					it('yet the owner can still resume', async () => {
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('nor can it do any other restricted action', async () => {
							...
							await assert.revert(systemStatus.resumeSystem({ from: account2 }));
							await assert.revert(
								systemStatus.suspendSynth(toBytes32('mimicETH'), '5', { from: account2 })
							);
							await assert.revert(systemStatus.resumeSynth(toBytes32('mimicETH'), { from: account2 }));
						});
					});
		...
		describe('suspendSynthExchange()', () => {
			...
			it('getSynthExchangeSuspensions(mimicETH, sBTC, iBTC) is empty', async () => {
				const { exchangeSuspensions, reasons } = await systemStatus.getSynthExchangeSuspensions(
					['mimicETH', 'sBTC', 'iBTC'].map(toBytes32)
				);
			...
			describe('when the owner suspends', () => {
				...
				it('getSynthExchangeSuspensions(mimicETH, sBTC, iBTC) returns values for sBTC', async () => {
					const { exchangeSuspensions, reasons } = await systemStatus.getSynthExchangeSuspensions(
						['mimicETH', 'sBTC', 'iBTC'].map(toBytes32)
					);
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('but not the others', async () => {
						...
						await systemStatus.requireSynthsActive(toBytes32('mimicETH'), sBTC);
					});
					it('and requireExchangeBetweenSynthsAllowed() reverts if one is the given synth', async () => {
						...
						await assert.revert(
							systemStatus.requireExchangeBetweenSynthsAllowed(toBytes32('mimicETH'), sBTC),
							reason
						);
						await assert.revert(
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('and all the require checks succeed', async () => {
							await systemStatus.requireSystemActive();
							await systemStatus.requireIssuanceActive();
							await systemStatus.requireExchangeBetweenSynthsAllowed(toBytes32('mimicETH'), sBTC);
							await systemStatus.requireSynthActive(sBTC);
							await systemStatus.requireSynthsActive(sBTC, toBytes32('mimicETH'));
							await systemStatus.requireSynthsActive(toBytes32('mimicETH'), sBTC);
						});
		...
		describe('suspendSynthsExchange()', () => {
			let txn;
			const [sBTC, mimicETH] = ['sBTC', 'mimicETH'].map(toBytes32);
			...
			describe('when the owner suspends', () => {
				const givenReason = '150';
				beforeEach(async () => {
					txn = await systemStatus.suspendSynthsExchange([sBTC, mimicETH], givenReason, {
						from: owner,
					});
				});
				...
				it('and for ETH', async () => {
					const { suspended, reason } = await systemStatus.synthExchangeSuspension(mimicETH);
					assert.equal(suspended, true);
					assert.equal(reason, givenReason);
					assert.eventEqual(txn.logs[1], 'SynthExchangeSuspended', [mimicETH, reason]);
				});
				it('getSynthExchangeSuspensions(mimicETH, sBTC, iBTC) returns values for mimicETH and sBTC', async () => {
					const { exchangeSuspensions, reasons } = await systemStatus.getSynthExchangeSuspensions(
						['mimicETH', 'sBTC', 'iBTC'].map(toBytes32)
					);
		...
		describe('resumeSynthsExchange()', () => {
			...
			describe('when the owner suspends', () => {
				const givenReason = '55';
				beforeEach(async () => {
					await systemStatus.suspendSynthsExchange([sBTC, mimicETH], givenReason, { from: owner });
				});
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						beforeEach(async () => {
							txn = await systemStatus.resumeSynthsExchange([sBTC, mimicETH], { from: account3 });
						});
						...
						it('and for sETH', async () => {
							...
							assert.equal(suspended, false);
							assert.equal(reason, '0');
							assert.eventEqual(txn.logs[1], 'SynthExchangeResumed', [mimicETH, givenReason]);
						});

						it('and all the require checks succeed', async () => {
							await systemStatus.requireSystemActive();
							await systemStatus.requireIssuanceActive();
							await systemStatus.requireExchangeBetweenSynthsAllowed(mimicETH, sBTC);
		...
		describe('suspendSynth()', () => {
			...
			it('getSynthSuspensions(mimicETH, sBTC, iBTC) is empty', async () => {
			...
			describe('when the owner suspends', () => {
				...
				it('getSynthSuspensions(mimicETH, sBTC, iBTC) returns values for sBTC', async () => {
					const { suspensions, reasons } = await systemStatus.getSynthSuspensions(
						['mimicETH', 'sBTC', 'iBTC'].map(toBytes32)
					);
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('and requireSynthsActive() reverts if one is the given synth', async () => {
						...
						await assert.revert(systemStatus.requireSynthsActive(toBytes32('mimicETH'), sBTC), reason);
						await assert.revert(systemStatus.requireSynthsActive(sBTC, toBytes32('sTRX')), reason);
						await systemStatus.requireSynthsActive(toBytes32('mimicETH'), toBytes32('mimicUSD')); // no issues
		...
		describe('suspendSynths()', () => {
			...
			describe('when the owner suspends', () => {
				const givenReason = '150';
				beforeEach(async () => {
					txn = await systemStatus.suspendSynths([sBTC, mimicETH], givenReason, { from: owner });
				});
				it('it succeeds for sBTC', async () => {
					const { suspended, reason } = await systemStatus.synthSuspension(sBTC);
					assert.equal(suspended, true);
				...
				it('and for sETH', async () => {
					...
					assert.equal(suspended, true);
					assert.equal(reason, givenReason);
					assert.eventEqual(txn.logs[1], 'SynthSuspended', [mimicETH, reason]);
				});
				it('getSynthSuspensions(mimicETH, sBTC, iBTC) returns values for both', async () => {
					const { suspensions, reasons } = await systemStatus.getSynthSuspensions(
						['mimicETH', 'sBTC', 'iBTC'].map(toBytes32)
					);
		...
		describe('resumeSynths()', () => {
			...
			describe('when the owner suspends', () => {
				const givenReason = '55';
				beforeEach(async () => {
					await systemStatus.suspendSynths([sBTC, mimicETH], givenReason, { from: owner });
				});
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						beforeEach(async () => {
							txn = await systemStatus.resumeSynths([sBTC, mimicETH], { from: account3 });
						});
						...
						it('and for sETH', async () => {
							...
							assert.equal(suspended, false);
							assert.equal(reason, '0');
							assert.eventEqual(txn.logs[1], 'SynthResumed', [mimicETH, givenReason]);
						});
		...
		describe('updateAccessControl()', () => {
			const synth = toBytes32('mimicETH');

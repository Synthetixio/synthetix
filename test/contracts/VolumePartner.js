...
contract('VolumePartner', accounts => {
	...
	describe('For normal exchanges', () => {
		...
		describe('Registering volume partner codes ', () => {
			it('Can register a volume partner code', async () => {
				...
				const transaction = await volumePartner.registerVolumePartnerCode(
					toBytes32('CODE1'),
					account1,
					toUnit('0.01'),
					{ from: owner }
				);
				assert.eventEqual(transaction, 'VolumePartnerCodeRegistered', {
					volumePartnerCode: toBytes32('CODE1'),
					owner: account1,
					caller: owner,
					feeRate: toUnit('0.01'),
				});
		...
		describe('Updating fee rate', () => {
			it('Can update when the owner', async () => {
				...
				const transaction = await volumePartner.updateFeeRate(toBytes32('CODE6'), toUnit('0.01'), {
					from: account1,
				});
				assert.eventEqual(transaction, 'FeeRateUpdated', {
					volumePartnerCode: toBytes32('CODE6'),
					caller: account1,
					feeRate: toUnit('0.01'),
				});
		...
		it('Can transfer ownership of a volume partner code', async () => {
			...
			const transaction1 = await volumePartner.nominateOwner(toBytes32('CODE9'), account2, {
				from: account1,
			});
			assert.eventEqual(transaction1, 'OwnerNominated', {
				volumePartnerCode: toBytes32('CODE9'),
				nominee: account2,
			});
			...
			const transaction2 = await volumePartner.acceptOwnership(toBytes32('CODE9'), {
				from: account2,
			});
			assert.eventEqual(transaction2, 'OwnershipAccepted', {
				volumePartnerCode: toBytes32('CODE9'),
				previousOwner: account1,
				newOwner: account2,
			});
		...
		it('Can accrue and claim fees from a normal exchange', async () => {
			...
			} = await exchanger.getAmountsForExchangeWithTrackingCode(
				amount,
				sUSD,
				sAUD,
				toBytes32('CODE11')
			...
			await synthetix.exchangeWithTracking(sUSD, amount, sAUD, ZERO_ADDRESS, toBytes32('CODE11'), {
				from: account1,
			});
			...
			const transaction = await volumePartner.claimFees(toBytes32('CODE11'), account2, {
				from: account1,
			});
			assert.eventEqual(transaction, 'FeesClaimed', {
				volumePartnerCode: toBytes32('CODE11'),
				caller: account1,
				recipient: account2,
				amount: expectedPartnerFee,
			});
	...
	describe('For atomic exchanges', () => {
		before(async () => {
			...
			await systemSettings.setPureChainlinkPriceForAtomicSwapsEnabled(sUSD, true, {
				from: owner,
			});
			await systemSettings.setPureChainlinkPriceForAtomicSwapsEnabled(sAUD, true, {
				from: owner,
			});
		});
		...
		it('Can accrue fees from an atomic exchange', async () => {
			...
			await synthetix.exchangeAtomically(sUSD, amount, sAUD, toBytes32('CODE12'), 0, {
				from: account1,
			});

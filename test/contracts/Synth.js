...
contract("Synth", async accounts => {
  ...
  it("should disallow a non-owner from setting the Synthetix contract", async () => {
    await assert.revert(
      XDRContract.setSynthetixProxy(account1, { from: account1 })
    );
  });
  ...
  it("should disallow a non-owner from setting the FeePool contract", async () => {
    await assert.revert(
      XDRContract.setFeePoolProxy(account1, { from: account1 })
    );
  });
  ...
  it("should transfer (ERC20) without error", async () => {
    ...
    const transaction = await sUSDContract.methods["transfer(address,uint256)"](
      account1,
      amount,
      {
        from: owner
  ...
  it("should revert when calling transferFrom (ERC20) with insufficient allowance", async () => {
    ...
    await assert.revert(
      sUSDContract.methods["transferFrom(address,address,uint256)"](
        owner,
        account1,
        amount,
        {
  ...
  it("should revert when calling transferFrom (ERC20) with insufficient balance", async () => {
    ...
    const amount = toUnit("10000");
    await synthetix.issueSynths(sUSD, amount.sub(web3.utils.toBN("1")), {
      from: owner
    });
  ...
  it("should revert when issue is called by non-Synthetix address", async () => {
    ...
    await assert.revert(
      XDRContract.issue(account1, toUnit("10000"), { from: owner })
    );
  });
  ...
  it("should burn successfully when called by Synthetix", async () => {
    ...
    const transaction = await XDRContract.burn(owner, toUnit("10000"), {
      from: owner
    });
  ...
  it("should revert when burn is called by non-Synthetix address", async () => {
    ...
    // Burning should fail.
    await assert.revert(
      XDRContract.burn(owner, toUnit("10000"), { from: owner })
    );
  });
  ...
  it("should revert when burning more synths than exist", async () => {
    ...
    await assert.revert(
      XDRContract.burn(owner, toUnit("10000").add(web3.utils.toBN("1")), {
        from: owner
      })
    );
  });

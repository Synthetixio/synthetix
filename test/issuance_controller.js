...
contract('Issuance controller', async function(accounts) {
  ...
  const addrs1 = accounts[6];
  const addrs2 = accounts[7];
  const addrs3 = accounts[8];
  const addrs4 = accounts[9];
  ...
  it('should set constructor params on deployment', async function () {
    ...
    instance = await IssuanceController.new(owner, fundsWallet, havven, nomin, oracle, usdEth, usdHav,  {
      from: deployerAcct
    });
  ...
  it('should set funds wallet when invoked by owner', async function () {
    let txn = await instance.setFundsWallet(addrs1, { from: owner });
    assert.equal(txn.logs[0].event, 'FundsWalletUpdated');
    assert.equal(txn.logs[0].args.newFundsWallet, addrs1);

    assert.equal(await instance.fundsWallet.call(), addrs1);
  });

  it('should not set funds wallet when not invoked by owner', async function () {
    try {
      await instance.setFundsWallet(addrs2, { from: deployerAcct });
    } catch (error) {
  ...
  it('should set oracle when invoked by owner', async function () {
    let txn = await instance.setOracle(addrs2, { from: owner });
    assert.equal(txn.logs[0].event, 'OracleUpdated');
    assert.equal(txn.logs[0].args.newOracle, addrs2);

    assert.equal(await instance.oracle.call(), addrs2);
  });

  it('should not set oracle when not invoked by owner', async function () {
    try {
      await instance.setOracle(addrs3, { from: deployerAcct });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }
  ...
  it('should set nomin when invoked by owner', async function () {
    let txn = await instance.setNomin(addrs3, { from: owner });
    assert.equal(txn.logs[0].event, 'NominUpdated');
    assert.equal(txn.logs[0].args.newNominContract, addrs3);

    assert.equal(await instance.nomin.call(), addrs3);
  });

  it('should not set nomin when not invoked by owner', async function () {
    try {
      await instance.setNomin(addrs4, { from: deployerAcct });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }
  ...
  it('should set havven when invoked by owner', async function () {
    ...
    let txn = await instance.setHavven(addrs4, { from: owner });
    assert.equal(txn.logs[0].event, 'HavvenUpdated');
    assert.equal(txn.logs[0].args.newHavvenContract, addrs4);

    assert.equal(await instance.havven.call(), addrs4);
  });

  it('should not set havven when not invoked by owner', async function () {
    
    try {
      await instance.setHavven(owner, { from: deployerAcct });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }
  ...
  it('should not set price stale period when not invoked by owner', async function () {
    ...
    try {
      await instance.setPriceStalePeriod(currentTimeInMillis, { from: deployerAcct });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }
  ...
  it('should set price stale period when invoked by owner', async function () {
    let currentTimeInMillis = new Date().getTime() + (60 * 60 * 1000);
    let txn = await instance.setPriceStalePeriod(currentTimeInMillis, { from: owner });
  ...
  it('should update prices when invoked by oracle', async function () {
    ...
    let txn = await instance.updatePrices(usdEth, usdHav, time, { from: addrs2 });
    let log = txn.logs[0];
  ...
  it('should not update prices if time sent is lesser than last updated price time', async function () {
    ...
    try {
      await instance.updatePrices(usdEth, usdHav, time, { from: addrs2 });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }
  ...
  it('should not update prices when not invoked by oracle', async function () {
    ...
    try {
      await instance.updatePrices(usdEth, usdHav, currentTimeInMillis, { from: addrs2 });
    } catch (error) {
      console.log(`Transaction error :: ${error}`);
      assert.include(error.message, 'revert');
    }

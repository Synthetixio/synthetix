...
class TestCourt(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Internal
        cls.setVotedYea = lambda self, sender, account, target: self.court.functions.publicSetVotedYea(account,
                                                                                                       target).transact(
            {'from': sender})
        cls.setVotedNay = lambda self, sender, account, target: self.court.functions.publicSetVotedNay(account,
                                                                                                       target).transact(
            {'from': sender})
    ...
    def test_beginConfiscationMotion(self):
        ...
        self.assertEqual(get_event_data_from_log(self.court_event_dict, tx_receipt.logs[0])['event'],
                         "ConfiscationVote")

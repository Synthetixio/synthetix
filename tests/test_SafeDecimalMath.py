...
class TestSafeDecimalMath(unittest.TestCase):
    ...
    def testArithmeticExpressions(self):
        self.assertEqual(
            self.safeSub(self.safeAdd(UNIT, self.safeDecDiv(self.safeDiv(self.safeAdd(UNIT, UNIT), 2), UNIT)),
                         self.safeDecMul(2 * UNIT, UNIT)), 0)
        self.assertEqual(
            self.safeDecDiv(self.safeDecMul(self.safeAdd(self.intToDec(1), UNIT), self.safeMul(2, UNIT)), UNIT // 2),
            self.intToDec(8))

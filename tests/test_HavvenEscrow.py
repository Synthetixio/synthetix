...
class TestHavvenEscrow(HavvenTestCase):
    ...
    def deployContracts(cls):
        sources = ["contracts/Havven.sol", "contracts/Nomin.sol",
        ...
        escrow_event_dict = generate_topic_event_map(
            cls.compiled['HavvenEscrow']['abi'])
        havven_event_dict = generate_topic_event_map(
            cls.compiled['PublicHavven']['abi'])
    ...
    def setUpClass(cls):
        ...
        cls.escrow = PublicHavvenEscrowInterface(
            cls.escrow_contract, "HavvenEscrow")

    def havven_updatePrice(self, sender, price, time):
        mine_tx(self.havven_contract.functions.updatePrice(
            price, time).transact({'from': sender}), 'updatePrice', 'Havven')
    ...
    def test_vestingQuantities(self):
        ...
        self.assertEqual(self.escrow.getVestingQuantity(
            alice, 0), quantities[0])

        for i in range(1, len(times)):
            self.escrow.appendVestingEntry(
                MASTER, alice, times[i], quantities[i])
        for i in range(1, len(times)):
            self.assertEqual(self.escrow.getVestingQuantity(
                alice, i), quantities[i])
    ...
    def test_vestingSchedules(self):
        ...
        self.assertEqual(self.escrow.vestingSchedules(
            alice, 0, 0), time + 1000)
    ...
    def test_balanceOf(self):
        ...
        self.assertEqual(self.escrow.balanceOf(
            alice), 100 * (2 ** k - 1) - 100)
    ...
    def test_totalVestedAccountBalance(self):
        ...
        self.assertEqual(self.escrow.totalVestedAccountBalance(
            alice), 100 * (2 ** k - 1))
        fast_forward(110)
        self.escrow.vest(alice)
        self.assertEqual(self.escrow.totalVestedAccountBalance(
    ...
    def test_maxVestingEntries(self):
        ...
        self.escrow.addRegularVestingSchedule(
            MASTER, alice, time + to_seconds(weeks=52), 100 * UNIT, 21)
        self.assertReverts(self.escrow.appendVestingEntry, MASTER,
                           alice, time + to_seconds(weeks=52, days=1), UNIT)
    ...
    def test_getVestingScheduleEntry(self):
        ...
        self.assertEqual(self.escrow.getVestingScheduleEntry(
            alice, 0), [time + 100, 1])
    ...
    def test_getNextVestingEntry(self):
        ...
        for i in range(len(entries)):
            self.escrow.appendVestingEntry(
                MASTER, alice, entries[i][0], entries[i][1])

        for i in range(len(entries)):
            fast_forward(to_seconds(weeks=1) + 30)
            self.assertEqual(
                self.escrow.getNextVestingEntry(alice), entries[i])
            self.escrow.vest(alice)
            self.assertEqual(self.escrow.getNextVestingEntry(alice), [
                             0, 0] if i == len(entries) - 1 else entries[i + 1])
    ...
    def test_getNextVestingTime(self):
        ...
        for i in range(len(entries)):
            fast_forward(to_seconds(weeks=1) + 30)
            self.assertEqual(
                self.escrow.getNextVestingTime(alice), entries[i][0])
            self.escrow.vest(alice)
            self.assertEqual(self.escrow.getNextVestingTime(
                alice), 0 if i == len(entries) - 1 else entries[i + 1][0])
    ...
    def test_getNextVestingQuantity(self):
        ...
        for i in range(len(entries)):
            fast_forward(to_seconds(weeks=1) + 30)
            self.assertEqual(
                self.escrow.getNextVestingQuantity(alice), entries[i][1])
            self.escrow.vest(alice)
            self.assertEqual(self.escrow.getNextVestingQuantity(
                alice), 0 if i == len(entries) - 1 else entries[i + 1][1])
    ...
    def test_escrowedFees(self):
        ...
        self.escrow.appendVestingEntry(
            MASTER, MASTER, block_time() + 100000, self.havven.totalSupply())

        self.havven_updatePrice(self.havven.oracle(),
                                UNIT, self.havven.currentTime() + 1)
    ...
    def test_withdrawHalfFees(self):
        self.havven.endow(MASTER, self.escrow.contract.address,
                          self.havven.totalSupply())
        self.escrow.appendVestingEntry(
            MASTER, MASTER, block_time() + 100000, self.havven.totalSupply() // 2)
        self.escrow.appendVestingEntry(
            MASTER, DUMMY, block_time() + 100000, self.havven.totalSupply() // 2)
    ...
    def test_appendVestingEntry(self):
        ...
        self.assertReverts(self.escrow.appendVestingEntry, MASTER, alice,
                           time + to_seconds(weeks=2), self.havven.totalSupply() + 1)

        # Should not be able to add a vestingEntry > balanceOf escrow account
        self.assertReverts(self.escrow.appendVestingEntry, MASTER,
                           alice, time + to_seconds(weeks=2), escrow_balance + 1)

        # Should not be able to vest in the past
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, 0, UNIT)
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, time - 1, UNIT)
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, time, UNIT)

        # Vesting quantities should be nonzero
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, time + to_seconds(weeks=2), 0)

        self.escrow.appendVestingEntry(
            MASTER, alice, time + to_seconds(weeks=2), amount)
        self.escrow.vest(alice)
        ...
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, time + to_seconds(days=1), amount)
        self.assertReverts(self.escrow.appendVestingEntry,
                           MASTER, alice, time + to_seconds(weeks=1), amount)
    ...
    def test_addVestingSchedule(self):
        ...
        self.assertEqual(
            self.escrow.getNextVestingQuantity(alice), quantities[0])
        ...
        self.assertEqual(
            self.escrow.getNextVestingQuantity(alice), quantities[1])
        ...
        self.assertEqual(
            self.escrow.getNextVestingQuantity(alice), quantities[2])
        ...
        time = block_time()
        # Bad (zero) quantities
        self.assertReverts(self.escrow.addVestingSchedule, MASTER, eve, [
                           time + 1000, time + 2000], [0, UNIT])
        self.assertReverts(self.escrow.addVestingSchedule, MASTER, eve, [
                           time + 1000, time + 2000], [UNIT, 0])

        # Bad times
        self.assertReverts(self.escrow.addVestingSchedule, MASTER, eve, [
                           time - 1000, time + 2000], [UNIT, UNIT])
        self.assertReverts(self.escrow.addVestingSchedule, MASTER, eve, [
                           time + 1000, time + 500], [UNIT, UNIT])
    ...
    def test_addRegularVestingSchedule(self):
        ...
        time = block_time()
        self.escrow.addRegularVestingSchedule(
            MASTER, alice, time + to_seconds(weeks=52), 100 * UNIT, 4)
        self.assertEqual(self.escrow.numVestingEntries(alice), 4)
    ...
    def test_addRegularVestingSchedule2(self):
        ...
        q = sum(self.escrow.getVestingQuantity(bob, i)
                for i in range(bob_periods))
        self.assertEqual(q, UNIT)
        self.assertEqual(self.escrow.getVestingTime(
            bob, bob_periods - 1), time, UNIT)

        self.assertReverts(self.escrow.addRegularVestingSchedule,
                           MASTER, carol, block_time() - 1, UNIT, 5)
        self.assertReverts(
            self.escrow.addRegularVestingSchedule, MASTER, carol, 0, UNIT, 5)
        self.assertReverts(self.escrow.addRegularVestingSchedule,
                           MASTER, carol, block_time() + 100000, UNIT, 0)
        ...
        self.assertEqual(self.escrow.numVestingEntries(tim),
                         self.escrow.numVestingEntries(pim))
        self.assertEqual(self.escrow.getVestingTime(tim, 0),
                         self.escrow.getVestingTime(pim, 0))
        self.assertEqual(self.escrow.getVestingQuantity(
            tim, 0), self.escrow.getVestingQuantity(pim, 0))
    ...
    def test_swap_havven(self):
        ...
        self.escrow.addRegularVestingSchedule(
            MASTER, alice, time + to_seconds(weeks=52), 100 * UNIT, 4)
        self.escrow.addRegularVestingSchedule(
            MASTER, bob, time + to_seconds(weeks=52), 100 * UNIT, 4)

        # Vest alice's first tranche as usual.
        fast_forward(to_seconds(weeks=13) + 10)
        self.escrow.vest(alice)
        self.assertEqual(self.havven.balanceOf(alice), 25 * UNIT)
        self.assertEqual(self.havven.balanceOf(
            self.escrow_contract.address), 175 * UNIT)

        print()
        # Deploy the new havven contract, with proxy and all.
        havven_proxy, _ = attempt_deploy(
            self.compiled, 'Proxy', MASTER, [MASTER])
        havven_contract, _ = attempt_deploy(self.compiled, 'PublicHavven', MASTER, [
                                            havven_proxy.address, self.havven_token_state.address, MASTER, MASTER, UNIT//2, [], ZERO_ADDRESS])
        proxied_havven = W3.eth.contract(
            address=havven_proxy.address, abi=self.compiled['PublicHavven']['abi'])
        ...
        self.assertEqual(new_havven.balanceOf(
            self.escrow_contract.address), 175 * UNIT)
        ...
        self.assertEqual(new_havven.balanceOf(bob), 5 * UNIT)
        self.assertEqual(new_havven.balanceOf(
            self.escrow_contract.address), 150 * UNIT)
        ...
        self.assertEqual(new_havven.balanceOf(bob), 5 * UNIT)
        self.assertEqual(new_havven.balanceOf(
            self.escrow_contract.address), 125 * UNIT)
        ...
        self.assertEqual(new_havven.balanceOf(bob), 5 * UNIT)
        self.assertEqual(new_havven.balanceOf(
            self.escrow_contract.address), 100 * UNIT)

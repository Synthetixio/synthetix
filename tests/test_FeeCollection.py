...
class TestHavven(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.n_poolFeeRate = lambda self: cls.nomin.functions.poolFeeRate_dec().call()
    ...
    def check_fees_collected(self, percentage_havvens, hav_holders, nom_users):
        ...
        self.assertClose(sum([self.h_balanceOf(addr) for addr in hav_addr]), int(h_total_supply * percentage_havvens),
                         precision=5)
        ...
        for addr in nom_addr:
            ...
            self.n_transfer(receiver, addr,
                            int(((self.n_balanceOf(receiver) * UNIT) // (self.n_transferFeeRate() + UNIT))))
    ...
    def check_fees_rolling_over(self, percentage_havvens, hav_holders, nom_users):
        ...
        for i in range(4):
            ...
            for addr in nom_addr:
                self.n_transfer(addr, receiver,
                                int(((self.n_balanceOf(addr) * UNIT) // (self.n_transferFeeRate() + UNIT))))
                self.n_transfer(receiver, addr,
                                int(((self.n_balanceOf(receiver) * UNIT) // (self.n_transferFeeRate() + UNIT))))
    ...
    def check_transferring_havven_fee_collection(self, h_percent, nom_users):
        ...
        for addr in nom_addr:
            self.n_transfer(addr, n_receiver,
                            int(((self.n_balanceOf(addr) * UNIT) // (self.n_transferFeeRate() + UNIT))))
            self.n_transfer(n_receiver, addr,
                            int(((self.n_balanceOf(n_receiver) * UNIT) // (self.n_transferFeeRate() + UNIT))))
